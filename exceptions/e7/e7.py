...
def guess_number():
    number_to_guess = randint(1, 500)
    int_number = -1
    tries = 0
    while int_number != number_to_guess:
        input_number = input("Next number: ")
        try:
            is_number(input_number)
        except ValueError as e:
            print(e)
        int_number = int(input_number)
        if int_number == number_to_guess:
            print("Congrats! you guess")
        elif int_number > number_to_guess:
            print("Lower")
        else:
            print("Higher")
        tries += 1
